...
def decode_regmax_distr(pred_dist, reg_max, anchor_points=None, xywh=True):
    """
    Convert predicted distributions (4 * regmax) to bounding boxes.

    Args:
        pred_dist (Tensor): (B, N, 4 * reg_max) raw output tensor.
        reg_max (int): The maximum distance bin.
        anchor_points (Tensor or None): (N, 2), anchor center positions (x, y).
        xywh (bool): Whether to return boxes in (xc, yc, w, h) or (x1, y1, x2, y2).

    Returns:
        Tensor: (B, N, 4) decoded boxes.
    """
    B, N, _ = pred_dist.shape
    assert pred_dist.shape[2] == 4 * reg_max, "Expected 4 * reg_max channels"
    ...
    # Project to scalar distance using expectation
    proj = torch.arange(reg_max, dtype=pred_prob.dtype, device=pred_prob.device)
    pred_ltrb = (pred_prob * proj).sum(dim=-1)  # (B, N, 4)

    if anchor_points is None:
        return pred_ltrb  # distances only

    # Reconstruct boxes from distances and anchor points
    return dist2bbox(pred_ltrb, anchor_points, xywh=xywh)  # (B, N, 4)
